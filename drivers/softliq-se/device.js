...
class softliqsdDevice extends Device {
   ...
   */
    ...
    async onDeviceUpdateParameters(deviceSerialNumber, data){
      ...
      if ( data.pregmode != undefined && this.getSetting('reg_mode') != !data.pregmode ){ 
        let settings = {
          reg_mode: !data.pregmode
        };
      ...
      if ( data.pregmo1 != undefined && this.decimalToString(this.getSetting('reg_mode_minutes')) != data.pregmo1.split(':')[1] ){ 
        ...
        this.setSettings(settings).catch(error => this.log("Error set reg_mode_minutes to value "+data.pregmo1+" Error: "+error.message));
      }
    }
    ...
    async onDeviceUpdateData(deviceSerialNumber, data){
      ...
      let capacity = parseInt( parseFloat(data.mrescapa1) );
      ...
      if (data.mregpercent1 != undefined){
      ...
      await this.homey.api.realtime("device_data_changed", {driver_id:'softliq-se', device_id: this.getData().id} );
  ...
  async onSettings({ oldSettings, newSettings, changedKeys }) {
    ...
    if ( oldSettings.reg_mode != newSettings.reg_mode ){
      ...
      await this.homey.app.deviceSetParameter(this.getData().id, "pregmode", !newSettings.reg_mode)
    }
